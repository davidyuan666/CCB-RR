...
class Experiment:
    ...
    def configureLogging(self):
        logging.basicConfig(
            level=logging.INFO,
            filename=("logs/log-{date:%Y-%m-%d_%H.%M.%S}__" + str(os.getpid()) + ".txt").format(
    ...
    def buildCommitFromRevrecRawCommit(self, rawCommit):
        ...
        commit["AuthorDate"] = datetime.datetime.utcfromtimestamp(rawCommit["timestamp"] / 1000).strftime(
            '%Y-%m-%d %H:%M:%S')
    ...
    def recommendReviewers(self, commit):
        ...
        if method == "rstrace+":
            with self.driver.session() as session:
                ...
                try:
                    result = session.run(
                        query, projectStartDate=parser.parse(self.projectStartDate, ignoretz=True),
                        endDate=parser.parse(commit["AuthorDate"], ignoretz=True),
                        modifiedFileNames=commit["modifiedFileNames"])
                except:
                    logging.exception("")
                    return []
    ...
    def addCommitAndRelationsToDB(self, commit):
        with self.driver.session() as session:
            ...
            session.run(
                "MERGE (a:Developer {id: $author}) MERGE (b:Commit {id: $commitId}) MERGE (a)-[:commits {createdAt: $date}]->(b)",
            ...
            for fileName in commit["modifiedFileNames"]:
                session.run(
                    "MERGE (a:Commit {id: $commitId}) MERGE (b:File {id: $fileName}) MERGE (a)-[:includes {createdAt: $date}]->(b)",
            ...
            for issueName in commit["relatedIssues"]:
                session.run(
                    "MERGE (a:Commit {id: $commitId}) MERGE (b:Issue {id: $issueName}) MERGE (a)-[:implements {createdAt: $date}]->(b)",
                    commitId=commit["id"], issueName=issueName, date=parser.parse(commit["AuthorDate"], ignoretz=True))

            # Add commit-reviewer relationship
            for reviewer in commit["reviewers"]:
                session.run(
                    "MERGE (a:Developer {id: $reviewer}) MERGE (b:Commit {id: $commitId}) MERGE (a)-[:reviews {createdAt: $date}]->(b)",
